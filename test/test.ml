...
let eqml a b =
  ...
  do rt := !rt lor (string_get a i lxor string_get b i) done;
  !rt = 0
...
let bench_eqaf hashes_0 hashes_1 n =
  Bench.Test.create_indexed
    ~name:"eqaf"
    ~args:(list_init n pred)
  @@ fun i ->
  let a = Array.get hashes_0 i in
  let b = Array.get hashes_1 i in
  (Staged.stage[@warning "-3"])
    (fun () -> eqaf a b)
...
let measurements hashes_0 hashes_1 =
  let run_config = Bench.Run_config.create
      ~time_quota:(Time.Span.of_sec 1.) () in
  Bench.measure ~run_config [ bench_eqaf hashes_0 hashes_1 tests
                            ; bench_eqst hashes_0 hashes_1 tests
                            ; bench_eqml hashes_0 hashes_1 tests ]

let analyzes hashes_0 hashes_1 =
  List.map
    (measurements hashes_0 hashes_1)
    ~f:(fun m -> match Bench.analyze ~analysis_configs:[ Bench.Analysis_config.nanos_vs_runs ] m with
        | Ok m -> m
        | Error exn ->
...
let kind_of_string x = match String.sub x 0 4 with
  | "eqaf" -> Eqaf
  | "eqst" -> Eqst
  | "eqml" -> Eqml
...
let times hashes_0 hashes_1 =
  List.map
    (analyzes hashes_0 hashes_1)
    ~f:(fun a -> let r = Analysis_result.regressions a in
         let c = Analysis_result.Regression.coefficients (Array.get r 0) in
         (kind_of_string (Analysis_result.name a)),
         Analysis_result.Coefficient.estimate (Array.get c 0))
...
module R = struct
  let hashes_0, hashes_1 =
    ...
    let hashes_0 = Array.init tests (fun i -> if i = where then same0 else random_hash digest_size i) in
    let hashes_1 = Array.init tests (fun i -> if i = where then same1 else random_hash digest_size i) in
...
module E = struct
  let hashes_0, hashes_1 =
    ...
    let hashes_1 = Array.init tests (fun i -> String.copy (Array.get hashes_0 i)) in
    hashes_0, hashes_1
...
let mean = function
  ...
  | lst ->
    let t, l = List.fold_left lst ~init:(0., 0.) ~f:(fun (t, l) x -> (x +. t), l +. 1.) in
    t /. l

let median lst =
  let arr = List.to_array lst in
  let len = Array.length arr in
    Array.sort ~compare:Float.compare arr
  ; ((Array.get arr ((len - 1) / 2)) +. (Array.get arr (len / 2))) /. 2.

let variance lst =
  let n = Float.of_int (List.length lst - 1) in
  let m = mean lst in
  let e = List.fold_left lst ~init:0. ~f:(fun a x -> a +. ((x -. m) *. (x -. m))) in
  sqrt ((1. /. n) *. e)

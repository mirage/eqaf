let get x i = Bytes.unsafe_get x i |> Char.code [@@inline]
external unsafe_get_int16 : bytes -> int -> int = "%caml_bytes_get16u" [@@noalloc]
...
let equal a b =
  let al = Bytes.length a in
  let bl = Bytes.length b in
  if al <> bl
  then false
